  ...
  included do
    ...
    def self.haruspex_autoinfer(hash, opts = {})
      ...
      unless hash.class == Hash
        raise "haruspex_autoinfer(): Expecting a hash, got a #{hash.class}"
      end
      ...
      DbSeedMagicSignature30dec22.each do |k, v|
        ...
        if (matching_methods[k][:all])
          if opts_verbose
            puts "🐦 haruspex_autoinfer(): Found match! #{matching_methods[k]}"
            io io io io
          end
    ...
    def self.private_parse_single_json(
      json_file,
      single_json_construct,
      description,
      my_class,
      method_name,
      ix,
    ...
    def self.generic_parse_array_of_jsons_from_file_with_method(
      json_file,
      description,
      my_class,
      method_name,
      ...
      if json_buridone.is_a? Array
        ...
        my_class, method_name =
          my_class.haruspex_autoinfer(single_json_construct) if method_name ==
          :haruspex_autoinfer
        ret =
          private_parse_single_json(
            json_file,
